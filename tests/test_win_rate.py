...
with known outcomes. Verifies the fix from the audit — win rate must
...
def _compute_win_rate(orders: list[dict]) -> dict:
    """
    Extracted from app.py /portfolio endpoint.
    Returns {"win_rate": float | None, "closed_trades": int, "wins": int}
    """
    ...
    return {"win_rate": win_rate, "closed_trades": len(closed_trades), "wins": len(wins)}
